...
/**
 * The sample workspace: pods, colleagues, contacts, campaigns and meetings, all named "Dummy ..."
 * so none of it can be mistaken for real data.
 *
 * It lives here, apart from `seed.ts`, because it is scaffolding for the test suites and for
 * looking around locally - never something a deployment runs. `seed.ts` loads it only when the
 * profile asks for it *and* the workspace is pointed at the built-in fake CRM rather than a real
 * Twenty, so there is no path by which any of this reaches a customer's database.
 */
/**
 ...
 */
export async function seedDemo(sequenceId: string, withCampaigns: boolean) {
