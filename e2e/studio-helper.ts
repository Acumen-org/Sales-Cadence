...
export async function beginStudio(page: Page, label: string, options: {count?:number;startDate?:string;endDate?:string}={}) {
 ...
 await page.getByLabel('Start date',{exact:true}).fill(options.startDate??'2027-01-04');await page.getByLabel('End date',{exact:true}).fill(options.endDate??'2027-01-08');
