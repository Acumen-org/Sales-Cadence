...
it('keeps long notes and nested BlockNote content through the final paragraph', () => {
  ...
  expect(normalizeNote({ bodyV2: { markdown: text } }, defaultTwentySchema).bodyMarkdown).toBe(text);
  const blocks = [{ id: 'one', content: [{ text: text }], children: [{ id: 'two', content: [{ type: 'link', content: [{ text: 'Nested decision' }] }] }] }];
  const note = normalizeNote({ bodyV2: { markdown: 'Short imported summary', blocknote: JSON.stringify(blocks) } }, defaultTwentySchema);
