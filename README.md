...
- **Tasks**: today / overdue / upcoming by type, filtered by pod and FO. Two ways to work: **List** for picking from a table, and **Task flow** for working a run one person at a time with a progress rail. The step's message is editable in place, so the template is a starting point you personalise before sending from your own mailbox. One row of controls with one panel beneath it: Done, Skip with a reason, Open in Twenty, and More for ending outreach. Legacy tasks also retain snooze and step controls; published-calendar tasks keep their approved dates. Call outcomes with notes, bulk actions across selected rows, keyboard shortcuts.
- **Campaign studio**: People, Outreach, Schedule. Choose the dates, audience and FOs with per-FO new-people/day limits. The studio generates a fitted starting outreach pattern, validates edits before opening the calendar, and reserves at most two different-stage batches per FO per weekday. Weekend adjustments and Clients / MIP / Tier 1 / Tier 2 / Tier 3 priority are built in. Selected contacts can use custom outreach groups authored inside that campaign; there is no separate sequence library. Publish the exact reviewed calendar, pause or stop work, or plan another run while retaining history. See [the scheduling model](CAMPAIGN-MODEL-PROPOSAL.md) and [release verification](CAMPAIGN-STUDIO-REVIEW.md).
- **Accounts**: one page per firm - the CRM record as labelled fields (industry, city, employees, AUM, owner, last synced), the people grouped into bands by the job titles Twenty holds, every call, email, meeting, task and campaign anyone there has been part of, and one timeline of the whole relationship. Filter to the accounts you own. An admin can **block** an account: it leaves Accounts, People, search, enrichment and campaign audiences, any sequence its people are in ends, and the whole list is managed in Settings > Blocked accounts. CRM people and companies are unchanged; existing mirrored tasks are cancelled. No record is deleted, so unblocking puts the account back.
