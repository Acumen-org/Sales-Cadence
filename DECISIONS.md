...
- **Steps are JSON on `Sequence.steps`** validated by a zod schema (`src/lib/sequences/steps.ts`). There are no versions. What a task actually says is frozen on the task itself (`Task.actionSnapshot`), which is what makes editing the plan under a live task safe; a step that has open tasks against it is refused outright (`StepInUseError`) so nothing shifts under work already handed out.
...
- **The next step is resolved by id, position as fallback.** It is the step following `Enrollment.currentStepId` in the plan as it stands now, so an edit made between two touches takes effect at the next touch and never retroactively.
- **A step holds several actions, and each is its own task**, sharing `stepId`. The task screen shows them as the modules of one touchpoint - a call and its follow-up email - and the step is done only when none of its modules is still pending.
...
- **The brief is assembled server-side per task**: person fields (email, phone, LinkedIn, where we met, pod, tags), enrollment (sequence, campaign, FO, start, clock shift), every module on this step with its own frozen copy, the last five `Touch` rows, the latest five Twenty notes and open opportunities fetched live (with a warning instead of an error when Twenty is unreachable), colleagues at the company with their enrollment status and FO, and the next step with its planned date.
...
- **Worker cadence**: continuous Twenty sync every `CRM_SYNC_SECONDS`, scheduler tick every `WORKER_TICK_SECONDS`, one reconcile plus a full cache refresh a day after `RECONCILE_HOUR` in the workspace timezone (guarded by its own audit row, so a restart does not repeat it), hourly session purge. The worker imports nothing from `next/*` so it runs under plain `tsx`.

## Sequences, Campaigns, People, Reports, Settings pages (phase 5)

- **The sequence editor saves the plan in place.** Modules are dragged into the order you want; saving validates (ascending business days, unique ids) under an advisory lock on the sequence, and refuses if any step being changed or moved has open tasks on it, naming how many. Step and action ids survive an edit (only new steps get new ids). Copy is literal text: there are no template variables to expand, no A/B variants and no either/or alternates.
...
- **People page** reads only the cache: name/company/email/title search, pod, tier, contact type and status filters, 100 per page, last touch from `Touch`, enrollment status from the latest enrollment. Enrolling happens through a campaign, or a bulk selection here; per-person pause, exit and reassign live on the campaign's enrollment rows, where the whole audience is in view.
- **Reports** roll up enrollments and tasks by pod, FO, campaign, sequence and channel. Reply rate = (replied + meetings) / (enrolled - exited); meeting rate = meetings / (enrolled - exited). Channel report splits DONE into observed-in-Twenty vs marked-manually so the team can see how much is being tracked automatically. The window is a From/Through filter with a 28-day default, and there are no overdue or stalled lists: what is overdue belongs in Tasks, where it can be worked. Admins see everything, Senior FOs their pods (plus anything assigned to them), Junior FOs only themselves (they are not offered the page).
...
- **Not adopted: A/B templates.** Outreach assigns variants at task creation and reports per-variant replies. Removed here on the owner's instruction, and it was the right call: a human writes each message from the step's copy before sending it, so the variant is not what varies and the statistic would measure nothing.
...
- Bugs found by the browser tests and fixed:
  ...
  - Keyboard `D` on a multi-module step opened the overflow menu instead of completing the first open module.
...
- **Instant sync, both directions.** Inbound: webhooks are processed the moment Twenty sends them; the nightly reconcile is only a safety net. Opening a person page re-reads that person from Twenty and refreshes the cache before rendering, so an edit made in the CRM seconds ago is visible without waiting for a webhook. There is no sync button to press: opening a person or an account is the refresh. Outbound: every write happens synchronously right after the database change that caused it, not on a timer.
- **What Cadence writes to Twenty, exhaustively**: (a) one activity Note on the person for every completed action, titled `[Cadence] Email 2 sent by Alisa` / `[Cadence] Call 1 made by Alisa - Left voicemail`, body with sequence, step, outcome and the FO's notes; (b) optionally a Twenty Task per open Cadence task (`Cadence: Email 1 - Dummy One`, assigned to the FO, due on the task day), updated to done or deleted when the Cadence task resolves. And (c) the contact and account fields in an Enrichment import that an admin or pod leader reviewed and applied (`email`, `phone`, `jobTitle`, `linkedinUrl`, `city`; `name`, `domain`, `industry`, `employees`, `city`, `linkedinUrl`, `aum`) - each row matched by id, email or domain, refused as a conflict if the CRM value moved since the preview, and verified against what Twenty returns. Nothing else: on its own initiative Cadence never edits person, company or opportunity fields, never sets `dnd` or `podOwner`, never touches notes or tasks it did not create. Its own opt-out and bad-data flags are local. Every write is logged in Settings > Activity log, and `CADENCE_DRY_RUN=true` turns all of them into log entries only.
...
- **An account is a Twenty company, and Cadence adds no fields of its own to it.** There was a Cadence-local relationship layer - who reports to whom, each contact's stance on the account, a free note - and it was removed. None of it could be derived from anything the CRM holds, so every value was either a guess or hand-maintained, and a hand-maintained chart is stale the first week nobody updates it.
- **The hierarchy shown is derived, not stored.** People are grouped into bands by the job titles Twenty already holds (executive, leadership, management, individual, title missing), and the panel says it is grouped from CRM job titles so nobody reads it as a reporting line. It is read-only and always current, because it is a view of the CRM rather than a copy of it.
