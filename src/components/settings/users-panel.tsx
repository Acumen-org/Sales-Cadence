...
export function UsersPanel({ users, pods }: { users: UserRow[]; pods: PodRow[] }) {
  ...
  return <div className="space-y-5">
    <Card title="Team" actions={<><label className="flex items-center gap-2 text-sm text-ink-700"><input type="checkbox" checked={showRemoved} onChange={(e) => setShowRemoved(e.target.checked)} />Show removed</label><button type="button" className="btn-primary btn-sm" onClick={() => setCreating(!creating)}>{creating ? 'Close' : 'Add team member'}</button></>}>
