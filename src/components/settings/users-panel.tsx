...
import { Avatar, Badge, Card, Count, Empty, Field, Info, type BadgeTone } from '@/components/ui';
...
const ROLES: Role[] = ['JUNIOR_FO', 'SENIOR_FO', 'POD_MANAGER', 'SALES_LEADER', 'BIZ_OPS', 'ADMIN'];
const ROLE_TONE: Record<Role, BadgeTone> = { ADMIN: 'purple', SALES_LEADER: 'purple', POD_MANAGER: 'purple', SENIOR_FO: 'blue', JUNIOR_FO: 'gray', BIZ_OPS: 'sky' };
...
function UserFields({ user, pods }: { user?: UserRow; pods: PodRow[] }) {
  return <div className="grid gap-4 md:grid-cols-2">
    ...
    <fieldset className="rounded-xl border border-line bg-white p-4 md:col-span-2"><legend className="px-1 text-sm font-medium text-ink-700">Pods<Info text="Required for Junior FO, Senior FO, Pod Manager and Sales Leader." /></legend><div className="flex flex-wrap gap-3">{pods.filter((pod) => !pod.archived).map((pod) => <label key={pod.id} className="flex cursor-pointer items-center gap-2 rounded-lg border border-line px-3 py-2 text-sm font-medium text-ink-900"><input type="checkbox" name="podIds" value={pod.id} defaultChecked={user?.podIds.includes(pod.id)} />{pod.name}</label>)}{!pods.some((pod) => !pod.archived) ? <span className="text-sm text-ink-500">No active pods</span> : null}</div></fieldset>
  </div>;
}
...
export function UsersPanel({ users, pods }: { users: UserRow[]; pods: PodRow[] }) {
  ...
  return <div className="space-y-5">
    <Card title="Team" actions={<><label className="flex items-center gap-2 text-sm text-ink-700"><input type="checkbox" checked={showRemoved} onChange={(e) => setShowRemoved(e.target.checked)} />Show removed</label><button type="button" className="btn-secondary btn-sm" onClick={() => setCreating(!creating)}>{creating ? 'Close' : 'Add team member'}</button></>}>
      ...
      <div className="overflow-x-auto"><table className="table"><thead><tr><th>Team member</th><th>Role</th><th>Pods</th><th>Access</th><th aria-label="Actions" /></tr></thead><tbody>{visibleUsers.map((u) => <Fragment key={u.id}>
        <tr><td><div className="flex items-center gap-3"><Avatar name={u.name} shape="circle" size={32} /><div><div className="font-medium text-ink-900">{u.name}</div><div className="mt-1 text-xs text-ink-500">{u.email}</div></div></div></td><td><Badge tone={ROLE_TONE[u.role]}>{ROLE_LABELS[u.role]}</Badge></td><td>{(() => { const mine = u.podIds.map((id) => pods.find((pod) => pod.id === id)).filter((pod): pod is PodRow => Boolean(pod && !pod.archived)); return mine.length ? <div className="flex flex-wrap gap-1">{mine.map((pod) => <Badge key={pod.id} tone="gray">{pod.name}</Badge>)}</div> : <Empty />; })()}</td><td><Badge tone={u.active ? 'green' : 'gray'}>{u.active ? 'Enabled' : 'Removed'}</Badge></td><td><div className="flex justify-end gap-2"><button type="button" className="btn-ghost btn-sm" onClick={() => { setEditing(editing === u.id ? null : u.id); setRemoving(null); }}>{editing === u.id ? 'Close' : 'Edit'}</button>{u.active ? <button type="button" className="btn-ghost btn-sm text-red-700" onClick={() => { setRemoving(removing === u.id ? null : u.id); setEditing(null); }}>Remove</button> : <ActionButton action={setUserAccessAction} payload={{ userId: u.id, active: 'true' }} className="btn-secondary btn-sm">Restore</ActionButton>}</div></td></tr>
