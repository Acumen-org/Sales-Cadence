...
export function PlannedCampaignDetail({ campaign: c, manager, enrollments, launchError, admin = false }: {
  ...
  return <div className="space-y-5 px-4 py-5 sm:px-6">
    ...
    <div className="grid grid-cols-2 gap-3 lg:grid-cols-4"><Stat label="People" value={c.personIds.length} /><Stat label="Touchpoints completed" value={completed} /><Stat label="Replies" value={enrollments.filter(e => e.repliedAt).length} /><Stat label="Meetings" value={enrollments.filter(e => e.meetingAt).length} /></div>
