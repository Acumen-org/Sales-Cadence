...
export function PlannedCampaignDetail({ campaign: c, manager, enrollments, launchError, admin = false }: {
  ...
  return <div className="space-y-5 px-4 py-5 sm:px-6">
    ...
    {late > 0 && <div role="status" className="rounded-xl border border-amber-200 bg-amber-50 p-5"><strong>{late} people have overdue touchpoints</strong><p className="mt-1 text-sm">Their next touchpoint waits for the current one. Published dates remain visible; missed work is not silently moved into another batch.</p></div>}
