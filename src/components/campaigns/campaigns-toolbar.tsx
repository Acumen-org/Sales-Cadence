...
export function CampaignsToolbar({ q, pods, sequences, fos, products, pod, sequence, fo, product, from, to }: Props) {
  ...
  const filterSelect = ({ name, value, label, all, items }: { name: string; value: string; label: string; all: string; items: { value: string; label: string }[] }) => (
  ...
  return (
    <div className="flex flex-1 flex-wrap items-center gap-2">
      ...
      {filterSelect({ name: 'pod', value: pod, label: 'Filter by pod', all: 'All pods', items: pods.map((p) => ({ value: p.id, label: p.name })) })}
      {fos.length ? filterSelect({ name: 'fo', value: fo, label: 'Filter by FO', all: 'All FOs', items: fos.map((f) => ({ value: f.id, label: f.name })) }) : null}
      ...
      <div id={panelId} className={`${showMore ? 'flex' : 'hidden'} w-full flex-wrap items-center gap-2 rounded-[10px] border border-line bg-canvas/70 p-2`}>
        {filterSelect({ name: 'product', value: product, label: 'Filter by product', all: 'Any product', items: products.map((p) => ({ value: p, label: optionLabel(p) })) })}
