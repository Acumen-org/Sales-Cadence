...
import { CampaignDraftSaveSchema, CampaignDraftSchema, calendarDays, outreachIsAutomatic, shortDateLabel, type CampaignDraft } from '@/lib/campaign-planner';
...
export function CampaignWorkspace({ initial, pods, products, campaignId, revision: initialRevision, canPublish = true, status, today }: { initial: CampaignDraft; pods: WorkspacePod[]; products: string[]; campaignId?: string; revision?: string; canPublish?: boolean; status?: string; today: string }) {
  ...
  const change = (next: CampaignDraft) => { setHold(null); generation.current++; edits.current++; dirty.current = true; setDraft(next); setReview(null); setSaveState(s => s === 'saving' ? s : 'idle'); setError(null); };
  ...
  const missing = [
    ...
    !(pace(draft.defaultBatchSize) && draft.fos.every(f => pace(f.batchSize))) && 'new people/day between 1 and 500',
  ...
  const plan = async (next: CampaignDraft, then?: () => void) => {
    ...
    setReview(r.data);
    if (!r.data.calendar.valid && r.data.stage !== 'outreach') return;
  ...
  const nextOutreach = () => {
    ...
    if (startPassed) { setHold(`The start date ${shortDateLabel(draft.startDate)} has passed.`); return; }
    void plan(draft, () => setTab(1));
  };
  const automatic = () => {
    if (!window.confirm('Replace this outreach with one the studio fits to the dates?')) return;
    setActiveFlow('default');
    void plan({ ...draft, flows: [{ id: 'default', name: 'Default', steps: outreachRecipe(1) }], assignments: {}, outreachEdited: false });
  };
  const go = (to: number) => {
  ...
  const applySuggestion = (next: CampaignDraft) => { void plan(next, tab === 0 ? () => setTab(1) : undefined); };
  ...
  return <div className="space-y-5 pb-4">
    ...
    {error && <div role="alert" className="flex flex-wrap items-center gap-3 rounded-lg border border-red-200 bg-red-50 px-4 py-3"><div className="min-w-0 flex-1"><span className="block font-medium text-red-900">Check these details</span><p className="mt-1 text-sm text-red-800">{error}</p></div></div>}
    {tab > 0 && planned && review && <PlanSummary review={review} pod={pod} onAddFo={addFo} onUntick={untick} onApply={applySuggestion} busy={pending || planning} />}
    ...
    {tab === 0 && <>
      <section className="surface grid gap-4 p-5 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Field label="Start date" required><input aria-label="Start date" type="date" value={draft.startDate} onChange={e => patch({ startDate: e.target.value })} className={`w-full ${startPassed ? '!border-red-300' : ''}`} />{startPassed && <button type="button" className="mt-1 text-xs font-medium text-red-700 hover:underline" onClick={startToday}>Start today</button>}</Field>
    ...
    {tab === 0 && review && !review.calendar.valid && review.stage === 'people' && <div ref={feedback} className="space-y-3"><PlannerFeedback review={review} onApply={applySuggestion} />{review.leftOut.length > 0 && <LeftOutList leftOut={review.leftOut} pod={pod} onAddFo={addFo} busy={pending} />}</div>}
    {tab === 1 && <div className="grid items-start gap-5 lg:grid-cols-[260px_minmax(0,1fr)]">
      <aside className="surface space-y-2 p-3 lg:sticky lg:top-4" aria-label="Outreach groups">
        ...
        {!reshapes(draft) && <button type="button" className="btn-ghost btn-sm w-full" onClick={automatic}>Let the studio set the outreach</button>}
    ...
    <footer className="sticky bottom-3 z-10 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-line bg-white/95 p-4 shadow-sm backdrop-blur">
      <span className="min-w-0 flex-1 text-sm text-ink-600">{tab === 0
        ? hold ? <span role="alert" className="text-red-700">{hold}</span> : <FooterCount people={draft.personIds.length} fos={draft.fos.length} />
        : tab === 1 && checking ? 'Checking fit…'
        : tab === 1 && review && !review.calendar.valid ? <span className="flex flex-wrap items-center gap-3"><span className="text-red-700">{review.calendar.issues[0]?.title}</span>{review.suggestions[0] && <button type="button" className="btn-secondary btn-sm" disabled={pending || planning} onClick={() => applySuggestion(review.suggestions[0].draft)}>{review.suggestions[0].label}</button>}</span>
        : planned ? <FooterCount people={planned.personIds.length} fos={planned.fos.length} /> : null}</span>
...
function PlanSummary({ review, pod, onAddFo, onUntick, onApply, busy }: { review: Review; pod?: WorkspacePod; onAddFo: (id: string) => void; onUntick: (ids: string[]) => void; onApply: (draft: CampaignDraft) => void; busy: boolean }) {
