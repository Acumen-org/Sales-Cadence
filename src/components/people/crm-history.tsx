  ...
  return <div className="space-y-3">
    <div id="crmEmails"><Card title="CRM emails" actions={<Badge tone="blue">Twenty</Badge>}>
      {!emails ? <div role="status" className="p-4 text-sm text-amber-800">Email history is temporarily unavailable.</div> : !emails.items.length ? <EmptyState title="No CRM emails" /> : <div className="divide-y divide-line">{emails.items.map((message) => {
        ...
        return <details key={message.id} className="group p-4"><summary className="flex cursor-pointer list-none items-start gap-3"><ActionIcon action="EMAIL" size={16} className="mt-0.5 shrink-0 text-brand-700" /><div className="min-w-0 flex-1"><div className="font-medium text-ink-900">{message.subject || 'No subject'}</div><div className="mt-1 text-xs text-ink-500">{senders.map((p) => p.displayName || p.handle).join(', ') || 'Sender unavailable'}</div></div><time dateTime={message.receivedAt} className="shrink-0 text-xs text-ink-600">{formatInstant(new Date(message.receivedAt), timezone)}</time></summary>
          <dl className="mt-3 space-y-2 rounded-lg bg-canvas p-3 text-xs"><div><dt className="text-ink-500">From</dt><dd className="mt-1 break-words font-medium text-ink-900">{senders.map((p) => p.handle).join(', ') || 'Unavailable'}</dd></div><div><dt className="text-ink-500">To / cc</dt><dd className="mt-1 break-words font-medium text-ink-900">{recipients.map((p) => p.handle).join(', ') || 'Unavailable'}</dd></div></dl>
    ...
    <div id="crmNotes"><Card title="CRM notes" actions={<Badge tone="blue">Twenty</Badge>}>
      {!notes ? <div role="status" className="p-4 text-sm text-amber-800">CRM notes are temporarily unavailable.</div> : !notes.items.length ? <EmptyState title="No CRM notes" /> : <div className="divide-y divide-line">{notes.items.map((note) => <details key={note.id} className="p-4"><summary className="flex cursor-pointer list-none items-start justify-between gap-3"><div className="font-medium text-ink-900">{note.title || 'Untitled note'}</div><time dateTime={note.createdAt} className="shrink-0 text-xs text-ink-600">{formatInstant(new Date(note.createdAt), timezone)}</time></summary><div className="mt-3 whitespace-pre-wrap break-words text-sm leading-7 text-ink-800">{note.bodyMarkdown || 'No note body'}</div>{note.createdByName ? <div className="mt-3 border-t border-line pt-3 text-xs"><span className="text-ink-500">Author</span><strong className="ml-2 text-ink-900">{note.createdByName}</strong></div> : null}</details>)}</div>}
