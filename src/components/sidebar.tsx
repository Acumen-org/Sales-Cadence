  ...
  return (
    <aside className="flex w-[228px] shrink-0 flex-col border-r border-line bg-[#fbfbfe]">
        ...
        {NAV.map((item) => {
          ...
          return (
            <Link key={item.href} href={item.href} className={clsx('nav-item', active && 'nav-item-active')}>
              <Icon size={18} className={active ? 'text-brand-700' : 'text-ink-400'} />
              {item.label}
            </Link>
          );
        })}
      ...
      <div className="space-y-2 px-3 pb-3">
        {mode === 'mock' ? (
          <div className="rounded-lg bg-amber-50 px-2.5 py-1.5 text-[11px] font-medium text-amber-800">Demo data (mock Twenty)</div>
        ) : null}
