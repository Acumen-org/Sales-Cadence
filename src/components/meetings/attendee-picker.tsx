  ...
  return <div className="space-y-3">
    ...
    <div className="relative">
      ...
      {open ? <div className="mt-2 max-h-72 overflow-y-auto rounded-xl border border-line bg-white p-1 shadow-surface" role="region" aria-label="Attendee search results">
        <div className="flex items-center justify-between px-2 py-1"><span className="text-xs text-ink-500">{loading ? 'Searching…' : 'People and team'}</span><button type="button" className="btn-ghost btn-sm" onClick={() => setOpen(false)}>Close</button></div>
