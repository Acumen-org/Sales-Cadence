...
export function AttendeePicker({ initial, name = 'attendeesJson' }: { initial: AttendeeSelection[]; name?: string }) {
  ...
  return <div className="space-y-3">
    ...
    {selected.length ? <ul className="divide-y divide-line rounded-xl border border-line">{selected.map((a) => <li key={keyOf(a)} className="flex items-center gap-3 p-3">
      <Avatar name={a.name ?? a.email ?? '?'} shape="circle" size={30} /><div className="min-w-0 flex-1"><div className="truncate text-sm font-semibold text-ink-900">{a.name ?? a.email}</div>{a.name && a.email ? <div className="truncate text-xs font-semibold text-ink-700">{a.email}</div> : null}</div>
