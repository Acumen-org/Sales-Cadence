  ...
  return <ActionForm action={action} onSuccess={r => { if (r.redirectTo) router.push(r.redirectTo); router.refresh(); }} className="space-y-5">
    ...
    {typeof duration === 'number' && duration > 0 ? (
      <div className="surface px-5 py-4">
        ...
        <div className="grid gap-1" style={{ gridTemplateColumns: `repeat(${Math.min(duration, 14)}, minmax(0, 1fr))` }} role="listbox" aria-label="Days of the sequence">
          {Array.from({ length: duration }, (_, d) => d + 1).map((day) => {
            ...
            return (
              ...
              >
                ...
                {weekStart ? <span className="absolute -top-2 left-0 text-[9px] uppercase tracking-wide text-ink-400">W{Math.floor((day - 1) / 7) + 1}</span> : null}
              </button>
            );
          })}
        </div>
