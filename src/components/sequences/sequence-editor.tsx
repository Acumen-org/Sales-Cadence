      ...
      {steps.map((step, i) => <li key={step.id} onDragOver={e => e.preventDefault()} onDrop={e => { e.preventDefault(); if (dragged !== null) move(dragged, i); setDragged(null); }}>
        ...
        <section className="surface overflow-hidden">
          <div className="flex flex-wrap items-center gap-3 border-b border-line bg-canvas/50 px-5 py-4">
            ...
            <label className="flex items-center gap-2 text-xs text-ink-500">Business day <input aria-label={'Step ' + (i + 1) + ' business day'} type="number" min={i ? steps[i - 1].day + 1 : 1} max={steps[i + 1] ? steps[i + 1].day - 1 : 999} value={step.day} onChange={e => update(i, { day: Number(e.target.value) })} disabled={locked(i)} className="!w-20" /></label>
