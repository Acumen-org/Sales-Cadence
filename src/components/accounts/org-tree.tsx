  ...
  return <div className="space-y-7">
    ...
    {grouped.map((band) => <section key={band.label} className="space-y-3">
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{band.people.map((p) => <article key={p.id} className="rounded-xl border border-line bg-white p-4 shadow-surface">
        ...
        <dl className="mt-4 grid grid-cols-2 gap-3 border-t border-line pt-3 text-xs"><div><dt className="text-ink-500">Touches</dt><dd className="mt-1 text-[20px] font-bold leading-none tracking-tight tabular-nums text-ink-900">{p.touches}</dd></div><div><dt className="text-ink-500">Last touch</dt><dd className="mt-1 font-semibold text-ink-900">{p.lastTouch ?? <span className="font-normal text-ink-300">-</span>}</dd></div>{p.enrollment ? <div className="col-span-2"><dt className="text-ink-500">Assigned to</dt><dd className="mt-1 font-semibold text-ink-900">{p.enrollment.foName}</dd></div> : null}</dl>
      </article>)}</div>
    </section>)}
  </div>;
}
