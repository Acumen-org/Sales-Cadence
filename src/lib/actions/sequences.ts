...
function parseRepeat(raw: FormDataEntryValue | null): { ok: true; days: number | null } | { ok: false; error: string } {
  ...
  if (!Number.isInteger(days) || days < 1 || days > 365) return { ok: false, error: 'Repeat every 1 to 365 working days, or leave it blank.' };
  return { ok: true, days };
}
...
export async function createSequenceAction(formData: FormData): Promise<ActionResult> {
  ...
  const { sequence } = await createSequence({ name: meta.data.name, description: meta.data.description ?? null, steps: steps.steps, repeatEveryDays: repeat.days }, userActor(admin));
...
export async function saveSequenceAction(formData: FormData): Promise<ActionResult> {
  ...
  try {
    ...
    const repeat = parseRepeat(formData.get('repeatEveryDays'));
    if (!repeat.ok) return repeat;
    await updateSequenceMeta(sequenceId, { name, archived: formData.get('archived') === 'on', repeatEveryDays: repeat.days }, userActor(admin));
  } catch (error) {
