...
export default async function CampaignDetailPage({params}:{params:Promise<{id:string}>}) {
 ...
 return <div className="space-y-5 px-6 pb-8 pt-2">
   <RecordHeader name={campaign.name} shape="square" badges={<Badge tone={campaign.status==='ACTIVE'?'green':'gray'}>{campaign.status==='ACTIVE'?'Running':campaign.status.toLowerCase().replace(/_/g,' ')}</Badge>} actions={<Link href="/campaigns" className="btn-secondary">All campaigns</Link>}/>
