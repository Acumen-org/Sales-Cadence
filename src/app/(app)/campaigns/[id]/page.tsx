...
export default async function CampaignDetailPage({params}:{params:Promise<{id:string}>}) {
 ...
 const [sequences,audience,preview]=await Promise.all([prisma.sequence.findMany({where:{archived:false},select:{id:true,name:true},orderBy:{name:'asc'}}), proposed ? prisma.personCache.findMany({where:{id:{in:campaign.personIds}},orderBy:{lastName:'asc'}}):Promise.resolve([]),
   ...
   proposed ? previewEnrollment({ personIds: campaign.personIds, sequenceId: campaign.sequenceId, podId: campaign.podId, campaignId: id, startDate: campaign.startDate, assignment: { mode: campaign.assignmentMode === 'ROUND_ROBIN' ? 'ROUND_ROBIN' : 'OWNER' }, dailyRampPerFo: campaign.dailyRampPerFo, actor: userActor(user) }).catch(() => null) : Promise.resolve(null)]);
 const conflictFor=new Map<string,EnrollConflict>((preview?.conflicts??[]).map(c=>[c.personId,c]));
 const foFor=new Map<string,string>((preview?.candidates??[]).map(c=>[c.personId,c.foName]));
 return <div className="space-y-5 px-6 pb-8 pt-2">
   ...
   {audience.length>0 && <Card title="Proposed audience" actions={preview ? <span className="flex items-center gap-2 text-[12px] text-ink-500"><Badge tone="green">{preview.candidates.length} will start</Badge>{preview.conflicts.length ? <Badge tone="amber">{preview.conflicts.length} skipped</Badge> : null}</span> : undefined}><div className="overflow-x-auto"><table className="table"><thead><tr><th>Person</th><th>Company</th><th>Email</th><th>FO</th><th>On launch</th></tr></thead><tbody>{audience.map(p=>{const c=conflictFor.get(p.id);return <tr key={p.id}><td><Link href={'/people/'+p.id}>{cachedPersonName(p)}</Link></td><td>{p.companyName??<Empty />}</td><td>{p.email??<Empty />}</td><td>{foFor.get(p.id)??<Empty />}</td><td>{c ? <span className="flex flex-wrap items-center gap-1.5"><Badge tone="amber">{enrollConflictLabel(c.reason)}</Badge>{c.detail ? <span className="text-[12px] text-ink-500">{c.detail}</span> : null}</span> : preview ? <Badge tone="green">Starts</Badge> : <Empty />}</td></tr>;})}</tbody></table></div></Card>}
