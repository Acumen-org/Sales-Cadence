...
export default async function EditCampaignPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ add?: string }> }) {
  ...
  return <div className="px-4 py-5 sm:px-6"><CampaignWorkspace canPublish={!c.followupSourceId || canApproveCampaign(user, c.podId)} campaignId={id} revision={c.updatedAt.toISOString()} pods={data.pods} products={[...defaultTwentySchema.personValues.productInterest]} initial={draft ?? {
    ...
    defaultBatchSize: c.startsPerFoPerDay ?? 20, fos: [], personIds: c.personIds, assignments: {}, flows: [{ id: 'default', name: 'Default', steps: parseSteps(c.sequence.steps) }],
  }} /></div>;
}
