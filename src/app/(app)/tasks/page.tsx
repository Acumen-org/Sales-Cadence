...
import { Avatar, Badge, EmptyState, Notice, RecordFields, Surface, Tabs, Toolbar, ViewHeader } from '@/components/ui';
...
export default async function TasksPage({ searchParams }: { searchParams: Promise<Search> }) {
  ...
  return <div className="space-y-4 px-6 pb-8 pt-2">
    <Surface flush><ViewHeader title="Tasks" /><Tabs inset={false} current={tab} tabs={(['today','overdue','upcoming','done'] as TaskTab[]).map(t => ({ key: t, label: TAB_LABELS[t], href: href({ tab: t, task: null }), count: counts[t] }))} />
