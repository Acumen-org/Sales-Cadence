...
export default async function SequencesPage({ searchParams }: { searchParams: Promise<{ q?: string; status?: string }> }) {
  ...
  return <PageFrame className="space-y-5 px-6 pb-8 pt-2">
    ...
    {!sequences.length && <div className="surface"><EmptyState
      title={q || status === 'archived' ? 'No matching sequences' : 'No sequences yet'}
      icon={<IconSequences size={22} />}
      action={q || status === 'archived'
        ? <Link href="/sequences" className="btn-secondary">Clear filters</Link>
        : canEditSequences(user) ? <Link href="/sequences/new" className="btn-primary"><IconPlus size={15} />New sequence</Link> : undefined}
    /></div>}
    {sequences.map(s => <Link key={s.id} href={'/sequences/' + s.id} className="surface group block overflow-hidden p-6 transition hover:border-brand-300 hover:shadow-md">
      <div className="mb-6 flex flex-wrap items-center gap-3"><span className="rounded-xl bg-brand-50 p-3 text-brand-700"><IconSequences size={22} /></span><h2 className="text-xl font-semibold tracking-tight group-hover:text-brand-700">{s.name}</h2><span className="ml-auto flex items-center gap-2">{s.repeatEveryDays ? <Badge tone="blue">Repeats every {s.repeatEveryDays} days</Badge> : null}<Badge tone="green">{s.campaigns} {s.campaigns === 1 ? 'campaign' : 'campaigns'}</Badge></span></div>
      {/* No connectors: every card is labelled with its own day, so the line added
          nothing except a stub trailing into white space wherever the flow wrapped. */}
      <div className="flex flex-wrap items-stretch gap-2.5">{s.preview.map((step) => <div key={step.id} className="flex min-w-32 flex-col rounded-xl border border-line bg-canvas/60 p-3"><div className="mb-2 text-xs text-ink-500">Day <strong className="text-ink-900">{step.day}</strong></div><div className="space-y-2">{step.actions.map((a, j) => <span key={j} className="flex items-center gap-2 text-xs font-medium text-ink-900"><ActionIcon action={a} size={14} />{ACTION_LABELS[a as ActionType]}</span>)}</div></div>)}</div>
    </Link>)}
  </PageFrame>;
}
